...
import { estimateExpiration, findInventoryMatches, findBloodReportMatch } from '@/app/actions';
...
const reportMatchingSchema = z.object({
  reportA: z.string().min(20, { message: 'Blood Report A is too short.' }),
  reportB: z.string().min(20, { message: 'Blood Report B is too short.' }),
});
...
export function AIForms() {
  ...
  const reportMatchingForm = useForm<ReportMatchingFormValues>({
    ...
    defaultValues: {
        reportA: 'Patient: John Doe, Blood Type: A+, Cross-match: Compatible, Antibodies: Negative.',
        reportB: 'Unit: #12345, Blood Type: A+, Cross-match: Compatible, Antibodies: Negative.',
    }
  });
  ...
  const onReportMatchingSubmit: SubmitHandler<ReportMatchingFormValues> = async (data) => {
    try {
      const result = await findBloodReportMatch(data);
          ...
          <form onSubmit={reportMatchingForm.handleSubmit(onReportMatchingSubmit)}>
            ...
            <CardContent className="space-y-4">
                <FormField
                    ...
                    name="reportA"
                    render={({ field }) => (<FormItem><FormLabel>Blood Report A (e.g., Patient)</FormLabel><FormControl><Textarea placeholder="Paste the first blood report here..." {...field} rows={5}/></FormControl><FormMessage /></FormItem>)}
                />
                <FormField
                    control={reportMatchingForm.control}
                    name="reportB"
                    render={({ field }) => (<FormItem><FormLabel>Blood Report B (e.g., Blood Unit)</FormLabel><FormControl><Textarea placeholder="Paste the second blood report here..." {...field} rows={5} /></FormControl><FormMessage /></FormItem>)}
                />
            </CardContent>
            <CardFooter className="flex-col items-start gap-4">
