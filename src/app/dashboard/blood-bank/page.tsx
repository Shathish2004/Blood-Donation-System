    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Please provide a reason for declining this request. This will be
    ...
    defaultValues: {
      ...
      bloodType:
        recipient.role === "Donor"
          ? recipient.bloodType
          : recipient.availableBloodTypes &&
            recipient.availableBloodTypes.length > 0
          ? recipient.availableBloodTypes[0]
          : "",
    },
  });
  ...
  const onSubmit: SubmitHandler<DirectRequestFormValues> = async (data) => {
    try {
      ...
      toast({
        variant: "destructive",
        title: "Error",
        description: e.message || "Failed to send direct request.",
      });
    }
  };
                    ...
                    >
                      ...
                      <SelectContent>
                        {recipient.role === "Donor" && recipient.bloodType ? (
                          <SelectItem value={recipient.bloodType}>
                            {recipient.bloodType}
                          </SelectItem>
                        ) : (
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    >
                      ...
                      <SelectContent>
                        {(
                          ["Low", "Medium", "High", "Critical"] as Urgency[]
                        ).map((level) => (
...
function AddEditUnitDialog({
  ...
  const onSubmit: SubmitHandler<UnitFormValues> = async (data) => {
    try {
      if (unit) {
        await updateBloodUnit(unit._id, {
          ...data,
        ...
        toast({
          title: "Success",
          description: "Blood unit has been updated.",
        });
      } else {
        await addBloodUnitToBank(
          {
            ...data,
            bloodType: data.bloodType as BloodType,
            location: bankEmail,
        ...
        toast({
          title: "Success",
          description: "New blood unit has been added.",
        });
      }
      onSave();
      setOpen(false);
      ...
      toast({
        variant: "destructive",
        title: "Error",
        description: "Failed to save blood unit.",
      });
    }
  };
      ...
      <DialogContent>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)}>
            <DialogHeader>
              <DialogTitle>
                {unit ? "Edit" : "Add New"} {donationType.replace(/_/g, " ")}{" "}
                Unit
              </DialogTitle>
              <DialogDescription>
                {unit
                  ? "Update the details for this blood unit."
                  : "Enter the details for the new blood unit."}
              </DialogDescription>
            </DialogHeader>
...
function AddEditTransferDialog({
  ...
  const onSubmit: SubmitHandler<TransferFormValues> = async (data) => {
    try {
      ...
      if (transfer) {
        ...
        toast({
          title: "Success",
          description: "Transfer record has been updated.",
        });
      } else {
        await addTransfer(transferData);
        toast({
          title: "Success",
          description: "New transfer has been recorded.",
        });
      }
      ...
      toast({
        variant: "destructive",
        title: "Error",
        description: e.message || "Failed to save transfer record.",
      });
    }
  };
  ...
  const partyPlaceholder =
    mode === "sent"
      ? "e.g., County Medical Center"
      : "e.g., Regional Blood Bank";
      ...
      <DialogContent>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)}>
            <DialogHeader>
              <DialogTitle>
                {dialogTitle} {mode === "sent" ? "Transfer" : "Received"} Record
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder={partyPlaceholder} {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
...
function CreateOfferDialog({
  ...
  const onSubmit: SubmitHandler<OfferFormValues> = async (data) => {
    try {
      ...
      toast({
        title: "Success",
        description: "Your blood offer has been posted.",
      });
      onSave();
      setOpen(false);
      form.reset({
      ...
      toast({
        variant: "destructive",
        title: "Error",
        description: e.message || "Failed to post offer.",
      });
    }
  };
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)}>
            <DialogHeader>
              ...
              <DialogDescription>
                Offer your surplus blood units to other facilities in the
                network.
              </DialogDescription>
            </DialogHeader>
            <div className="py-4 space-y-4">
...
function BloodBankPageContent() {
  ...
  const [mySentRequestResponses, setMySentRequestResponses] = useState<
    Notification[]
  >([]);
  const [requestHistory, setRequestHistory] = useState<BloodUnit[]>([]);
  ...
  const selectedState = filters.state as
    | keyof typeof indianStatesAndDistricts
    | undefined;
    ...
    try {
      ...
      setIncomingRequests(
        notifications.filter(
          (n) => n.type === "request" || n.type === "emergency"
        )
      );
      setMySentRequestResponses(
        notifications.filter((n) =>
          ["response", "decline", "claim"].includes(n.type)
        )
      );
      setRequestHistory(historyData);
      setPotentialFacilities(
        facilitiesData.filter(
          (f) => f.role === "Hospital" || f.role === "Blood Bank"
        )
      );
      ...
      toast({
        variant: "destructive",
        title: "Error",
        description: "Failed to fetch blood bank data.",
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleClaimOffer = async (offerId: string) => {
    ...
    try {
      ...
      toast({
        variant: "destructive",
        title: "Error",
        description: e.message || "Failed to claim offer.",
      });
    }
  };
  ...
  const handleCancelOffer = async (offerId: string) => {
    ...
    try {
      ...
      toast({
        variant: "destructive",
        title: "Error",
        description: e.message || "Failed to cancel offer.",
      });
    }
  };
  ...
  const filteredFacilities = React.useMemo(() => {
    return potentialFacilities.filter((facility) => {
      if (facility.email === user?.email) return false;
      ...
      const cityMatch =
        !appliedFilters.city ||
        normalize(facility.city).includes(normalize(appliedFilters.city));
      const stateMatch =
        !appliedFilters.state ||
        normalize(facility.state).includes(normalize(appliedFilters.state));
      const roleMatch =
        !appliedFilters.role || facility.role === appliedFilters.role;
      const bloodTypeMatch =
        !appliedFilters.bloodType ||
        (facility.availableBloodTypes &&
          facility.availableBloodTypes.includes(appliedFilters.bloodType));
      const donationTypeMatch =
        !appliedFilters.donationType ||
        ((facility.inventorySummary?.whole_blood ?? 0) > 0 &&
          appliedFilters.donationType === "whole_blood") ||
        ((facility.inventorySummary?.plasma ?? 0) > 0 &&
          appliedFilters.donationType === "plasma") ||
        ((facility.inventorySummary?.red_blood_cells ?? 0) > 0 &&
          appliedFilters.donationType === "red_blood_cells");
  ...
  }, [potentialFacilities, appliedFilters, user]);
      ...
      <Card>
        <CardHeader className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div>
            ...
            <CardDescription>
              Manage your {typeLabel.toLowerCase()} stock.
            </CardDescription>
          </div>
          <AddEditUnitDialog
          ...
          <div className="md:hidden space-y-4">
            {filteredInventory.length > 0 ? (
                  ...
                  <Card key={unit._id} className="p-4">
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        <Badge
                          variant="outline"
                          className="text-primary border-primary/50 text-lg"
                        >
                          {unit.bloodType}
                        </Badge>
                        {unit.donationType && (
                          <p className="text-sm text-muted-foreground mt-1 capitalize">
                            {unit.donationType.replace(/_/g, " ")}
                          </p>
                        )}
                    ...
                    <div className="space-y-2 text-sm mb-4">
                      <div className="flex justify-between">
                        ...
                        <span>
                          {format(parseISO(unit.collectionDate), "PP")}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="font-medium text-muted-foreground">
                        ...
                        <span>
                          {format(parseISO(unit.expirationDate), "PP")}
                        </span>
                      </div>
                    </div>
              ...
              <div className="text-center py-10">
                No {typeLabel.toLowerCase()} inventory found.
              </div>
            )}
          </div>
  ...
  const renderTransferHistory = (
    ...
    const history = mode === "sent" ? sentTransfers : receivedTransfers;
    const filteredHistory = history.filter(
      (t) => t.donationType === donationType
    );
    const typeLabel = donationType.replace(/_/g, " ");
        ...
        <CardHeader className="flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2">
              ...
              <CardTitle>
                {mode === "sent" ? "Distribution" : "Received"} History -{" "}
                <span className="capitalize">{typeLabel}</span>
              </CardTitle>
            </div>
            <CardDescription>
              {mode === "sent"
                ? `Log of ${typeLabel} distributed to other facilities.`
                : `Log of ${typeLabel} received from other facilities.`}
            </CardDescription>
          </div>
          <AddEditTransferDialog
            user={user}
            mode={mode}
            onSave={handleSave}
            donationType={donationType}
        ...
        <CardContent>
          <div className="md:hidden space-y-4">
            {filteredHistory.length > 0 ? (
                  ...
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <p className="font-bold">
                        {mode === "sent" ? item.destination : item.source}
                      </p>
              ...
              <div className="text-center py-10">
                No {mode === "sent" ? "sent" : "received"} {typeLabel} transfers
                recorded.
              </div>
            )}
          </div>
                ...
                {filteredHistory.length > 0 ? (
                  ...
                  <TableRow>
                    <TableCell colSpan={5} className="h-24 text-center">
                      No {mode === "sent" ? "sent" : "received"} {typeLabel}{" "}
                      transfers recorded.
                    </TableCell>
                  </TableRow>
                )}
    ...
    <div className="space-y-6">
      {view === "inventory" && (
        <Tabs defaultValue="whole_blood" className="w-full">
          ...
          <TabsContent value="whole_blood" className="mt-4">
            {renderInventoryTable("whole_blood")}
          </TabsContent>
          <TabsContent value="plasma" className="mt-4">
            {renderInventoryTable("plasma")}
          </TabsContent>
          <TabsContent value="red_blood_cells" className="mt-4">
            {renderInventoryTable("red_blood_cells")}
          </TabsContent>
        </Tabs>
      )}
      ...
      {view === "requests" && (
        <div className="space-y-6">
          <Card className="border-destructive">
            ...
            <CardContent>
              {incomingRequests.filter((req) => req.type === "emergency")
                .length > 0 ? (
                ...
                <div className="text-center h-24 flex items-center justify-center">
                  <p className="text-muted-foreground">
                    No current emergency broadcasts.
                  </p>
                </div>
              )}
            </CardContent>
          ...
          <Card>
            ...
            <CardContent>
              ...
              <div className="space-y-4 md:hidden">
                {incomingRequests.filter((req) => req.type === "request")
                  .length > 0 ? (
                  incomingRequests
            ...
            <CardContent>
              ...
              <div className="space-y-4 md:hidden">
                {mySentRequestResponses.length > 0 ? (
                  ...
                  <div className="text-center h-24 flex items-center justify-center">
                    <p className="text-muted-foreground">
                      No new responses or notifications.
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                >
                  <SelectTrigger>
                    <SelectValue
                      placeholder={
                        filters.state ? "District" : "Select a state first"
                      }
                    />
                  </SelectTrigger>
                  <SelectContent>
              ...
              {filteredFacilities.map((facility) => (
                <Card key={facility._id} className="p-4">
                  <div className="flex justify-between items-start mb-2">
                    <div>
                      ...
                      <div className="mt-2 space-y-1 text-xs text-muted-foreground">
                        ...
                        <p className="flex items-center gap-1">
                          <Phone className="h-3 w-3" /> {facility.mobileNumber}
                        </p>
                      </div>
                    </div>
                    <Badge
                  ...
                  <div className="text-sm space-y-1 my-4">
                    {facility.inventorySummary ? (
                      <>
                        <p>
                          <span className="font-semibold">WB:</span>{" "}
                          {facility.inventorySummary.whole_blood} units
                        </p>
                        <p>
                          <span className="font-semibold">Plasma:</span>{" "}
                          {facility.inventorySummary.plasma} units
                        </p>
                        <p>
                          <span className="font-semibold">RBC:</span>{" "}
                          {facility.inventorySummary.red_blood_cells} units
                        </p>
                        ...
                        <TableCell className="text-xs">
                          {facility.inventorySummary ? (
                            <div className="flex flex-col">
                              ...
                              <span>
                                Plasma: {facility.inventorySummary.plasma} units
                              </span>
                              <span>
                                Red Blood Cells:{" "}
                                {facility.inventorySummary.red_blood_cells}{" "}
                                units
                              </span>
                            </div>
                          ) : (
      ...
      {view === "polls" && (
        <div className="space-y-6">
          <Card>
            <CardHeader className="flex-col md:flex-row items-start md:items-center justify-between gap-4">
              <div>
                ...
                <CardDescription>
                  Offers for surplus blood from other facilities.
                </CardDescription>
              </div>
            ...
            <CardContent>
              ...
              <div className="space-y-4 md:hidden">
                {bloodOffers
                  .filter(
                    (o) =>
                      o.creatorEmail !== user.email && o.status === "Available"
                  )
                  .map((offer) => (
                    <Card key={offer._id} className="p-4">
                      <div className="flex justify-between items-start mb-2">
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">
                            {formatDistanceToNow(parseISO(offer.date), {
                              addSuffix: true,
                            })}
                          </p>
                        </div>
                      ...
                      <p className="text-sm capitalize font-medium mb-2">
                        {offer.donationType.replace(/_/g, " ")}
                      </p>
                      {offer.message && (
                        <p className="text-sm text-muted-foreground mb-4">
                          {offer.message}
                ...
                {bloodOffers.filter(
                  ...
                  <div className="text-center h-24 flex items-center justify-center">
                    <p>No available offers from other facilities.</p>
                  </div>
                )}
              </div>
              {/* Desktop View */}
                ...
                <Table>
                  ...
                  <TableBody>
                    ...
                    {bloodOffers.filter(
                      ...
                      <TableRow>
                        <TableCell colSpan={7} className="h-24 text-center">
                          No available offers from other facilities.
                        </TableCell>
                      </TableRow>
                    )}
                  </TableBody>
          ...
          <Card>
            <CardHeader>
              <CardTitle>My Posted Polls</CardTitle>
            ...
            <CardContent>
              ...
              <div className="space-y-4 md:hidden">
                {bloodOffers
                  .filter((o) => o.creatorEmail === user.email)
                  .map((offer) => (
                    <Card key={offer._id} className="p-4">
                      <div className="flex justify-between items-start mb-4">
                        <div>
                        ...
                        <Badge
                          variant={
                            offer.status === "Claimed" ? "default" : "secondary"
                          }
                          className={cn(
                            offer.status === "Claimed" && "bg-green-600"
                          )}
                        >
                          {offer.status}
                        </Badge>
                ...
                {bloodOffers.filter((o) => o.creatorEmail === user.email)
                  .length === 0 && (
                  <div className="text-center h-24 flex items-center justify-center">
                    <p>You have not posted any offers.</p>
                  </div>
                )}
              </div>
              {/* Desktop View */}
              ...
              {requestHistory.map((unit) => (
                <Card key={unit._id} className="p-4">
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <p className="font-bold">
                        {unit.locationName || unit.location}
                      </p>
                      <p className="text-sm text-muted-foreground">
                        {unit.units} units of {unit.bloodType}
                      </p>
                    </div>
                    <Badge
                      variant={"secondary"}
                      className={cn(
                        getUnitStatus(unit.expirationDate).color,
                        "text-white"
                      )}
                    >
                  ...
                  <div className="flex justify-between items-center text-sm">
                    <span>
                      Expires: {format(parseISO(unit.expirationDate), "PP")}
                    </span>
                  </div>
                </Card>
              ))}
                    ...
                    return (
                      <TableRow key={unit._id}>
                        <TableCell>
                          ...
                          <div className="text-xs text-muted-foreground">
                            {unit.locationEmail || "N/A"}
                          </div>
                          <div className="text-xs text-muted-foreground">
                            {unit.locationMobile || "N/A"}
                          </div>
                        </TableCell>
                        <TableCell>
                          <Badge
      ...
      {view === "transfer-history" && (
        <Tabs defaultValue="whole_blood" className="w-full">
          ...
          <TabsContent value="whole_blood" className="mt-4">
            {renderTransferHistory("sent", "whole_blood")}
          </TabsContent>
          <TabsContent value="plasma" className="mt-4">
            {renderTransferHistory("sent", "plasma")}
          </TabsContent>
          <TabsContent value="red_blood_cells" className="mt-4">
            {renderTransferHistory("sent", "red_blood_cells")}
          </TabsContent>
        </Tabs>
      )}
      ...
      {view === "received-history" && (
        <Tabs defaultValue="whole_blood" className="w-full">
          ...
          <TabsContent value="whole_blood" className="mt-4">
            {renderTransferHistory("received", "whole_blood")}
          </TabsContent>
          <TabsContent value="plasma" className="mt-4">
            {renderTransferHistory("received", "plasma")}
          </TabsContent>
          <TabsContent value="red_blood_cells" className="mt-4">
            {renderTransferHistory("received", "red_blood_cells")}
          </TabsContent>
        </Tabs>
      )}
