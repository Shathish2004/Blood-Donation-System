...
import { addBloodUnitToBank, deleteBloodUnitFromBank, getBloodBankInventory, getNotificationsForUser, getPotentialDonors, getUser, User, respondToRequest, declineRequest, createDirectBloodRequest, addTransfer, updateTransfer, getSentTransfers, getReceivedTransfers, createBloodOffer, getBloodOffers, claimBloodOffer, cancelBloodOffer, updateBloodUnit, getBloodRequestsForUser, getAllInventory } from '@/app/actions';
import { useToast } from '@/hooks/use-toast';
import type { BloodUnit, Notification, Urgency, BloodRequest, Transfer, BloodOffer, DonationType, BloodType } from '@/lib/types';
...
function AddEditUnitDialog({ unit, bankEmail, onSave, donationType }: { unit?: BloodUnit, bankEmail: string, onSave: () => void, donationType: DonationType }) {
  const { toast } = useToast();
  const [open, setOpen] = useState(false);
  ...
  const onSubmit: SubmitHandler<UnitFormValues> = async (data) => {
    try {
      if (unit) {
        ...
        await addBloodUnitToBank({ ...data, bloodType: data.bloodType as BloodType, location: bankEmail, collectionDate: data.collectionDate.toISOString() }, bankEmail);
...
function BloodBankPageContent() {
  ...
  const [mySentRequestResponses, setMySentRequestResponses] = useState<Notification[]>([]);
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      setPotentialFacilities(facilitiesData.filter(f => (f.role === 'Hospital' || f.role === 'Blood Bank')));
  ...
  const filteredFacilities = React.useMemo(() => {
    return potentialFacilities.filter(facility => {
        ...
        const cityMatch = !appliedFilters.city || normalize(facility.city).includes(normalize(appliedFilters.city));
        const stateMatch = !appliedFilters.state || normalize(facility.state).includes(normalize(appliedFilters.state));
        const roleMatch = !appliedFilters.role || facility.role === appliedFilters.role;
        const bloodTypeMatch = !appliedFilters.bloodType || (facility.availableBloodTypes && facility.availableBloodTypes.includes(appliedFilters.bloodType));
        const donationTypeMatch = !appliedFilters.donationType || (
          (facility.inventorySummary?.whole_blood ?? 0) > 0 && appliedFilters.donationType === 'whole_blood' ||
          (facility.inventorySummary?.plasma ?? 0) > 0 && appliedFilters.donationType === 'plasma' ||
          (facility.inventorySummary?.red_blood_cells ?? 0) > 0 && appliedFilters.donationType === 'red_blood_cells'
        );
    ...
    return (
        <Card>
          <CardHeader className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
              ...
              <AddEditUnitDialog bankEmail={user.email!} onSave={handleSave} donationType={donationType} />
            </CardHeader>
            <CardContent>
              {/* Mobile View */}
                    ...
                    return (
                      <Card key={unit._id} className="p-4">
                        ...
                        <div className="flex justify-end gap-2">
                          <AddEditUnitDialog unit={unit} bankEmail={user.email!} onSave={handleSave} donationType={donationType} />
                    ...
                    {filteredInventory.length > 0 ? filteredInventory.map((unit) => {
                      ...
                      return (
                        <TableRow key={unit._id}>
                          ...
                          <TableCell className="capitalize">{unit.donationType?.replace(/_/g, ' ')}</TableCell>
                ...
                {incomingRequests.filter(req => req.type === 'request').length > 0 ? incomingRequests.filter(req => req.type === 'request').map(req => (
                    <Card key={req._id} className="p-4">
                        <div className="flex justify-between items-start mb-2">
                              ...
                              <Badge variant="outline" className={cn( req.type === 'request' && 'border-blue-500 text-blue-500' )}>
                                  {req.type}
                              </Badge>
                        </div>
              ...
              {filteredFacilities.map((facility) => (
                    <Card key={facility._id} className="p-4">
                            ...
                            <div>
                              ...
                              <p className="text-sm text-muted-foreground">{facility.city}, {facility.state}</p>
                               <div className="mt-2 space-y-1 text-xs text-muted-foreground">
                                    <p className="flex items-center gap-1"><Mail className="h-3 w-3" /> {facility.email}</p>
                                    <p className="flex items-center gap-1"><Phone className="h-3 w-3" /> {facility.mobileNumber}</p>
                                </div>
                            </div>
                            <Badge variant={facility.role === 'Blood Bank' ? 'secondary' : 'outline'}>{facility.role}</Badge>
                      </div>
                      <div className="text-sm space-y-1 my-4">
                            {facility.inventorySummary ? (
                                <>
                                <p><span className="font-semibold">WB:</span> {facility.inventorySummary.whole_blood} units</p>
                                <p><span className="font-semibold">Plasma:</span> {facility.inventorySummary.plasma} units</p>
                                <p><span className="font-semibold">RBC:</span> {facility.inventorySummary.red_blood_cells} units</p>
                                <div className="flex flex-wrap gap-1 pt-2">
                                    {facility.availableBloodTypes?.map(type => <Badge key={type} variant="outline">{type}</Badge>)}
                                </div>
                                </>
                            ) : (
                      ...
                      <div className="flex justify-end mt-4">
                          <DirectRequestDialog recipient={facility} requester={user!} onSuccess={handleSave} />
                      </div>
                    </Card>
                  ))}
                {filteredFacilities.length === 0 && <div className="text-center h-24 flex items-center justify-center"><p>No other facilities found.</p></div>}
            </div>
                      ...
                      filteredFacilities.map((facility) => (
                        <TableRow key={facility._id}>
                           <TableCell>
                              ...
                              <div className="text-xs text-muted-foreground flex items-center gap-1"><Mail className="h-3 w-3" />{facility.email}</div>
                              <div className="text-xs text-muted-foreground flex items-center gap-1"><Phone className="h-3 w-3" />{facility.mobileNumber}</div>
                            </TableCell>
                          <TableCell>
                            <Badge
                              variant={
                             ...
                             {facility.inventorySummary ? (
                                 <div className="flex flex-col">
                                     <div className="flex flex-wrap gap-1 pb-1">
                                         {facility.availableBloodTypes?.map(type => <Badge key={type} variant="outline" className="text-xs">{type}</Badge>)}
                                     </div>
                                     <span>Whole Blood: {facility.inventorySummary.whole_blood} units</span>
                                     <span>Plasma: {facility.inventorySummary.plasma} units</span>
                                     <span>Red Blood Cells: {facility.inventorySummary.red_blood_cells} units</span>
                                 </div>
                             ) : (
